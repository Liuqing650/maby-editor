...
function onlyText(opts) {
  return {
     ...
     */
    normalize(transform, node, { toRemove = List(), toAdd = List(), toJoin = List() }) {
      // Remove invalids
      transform = toRemove.reduce((t, child) => {
        return t.removeNodeByKey(child.key, { normalize: false });
      }, transform);

      // Join nodes.
      const pairs = toJoin.butLast()
        .map((child, index) => [child.key, toJoin.get(index + 1).key]);

      // Join every node onto the previous one.
      transform = pairs.reverse().reduce((t, [childKey, nextChildKey]) => {
        return t.joinNodeByKey(nextChildKey, childKey, { normalize: false });
      }, transform);

      // Add missing nodes
      transform = toAdd.reduce((t, child) => {
